...
def pose():
    ...
    pose_out3 = tf.reduce_mean(input_tensor=tf.stack([tf.image.resize(pose_out3_100, tf.shape(input=image_batch_origin)[1:3,], method=tf.image.ResizeMethod.NEAREST_NEIGHBOR),
                                         tf.image.resize(pose_out3_125, tf.shape(input=image_batch_origin)[1:3,], method=tf.image.ResizeMethod.NEAREST_NEIGHBOR),
                                         tf.image.resize(pose_out3_075, tf.shape(input=image_batch_origin)[1:3,], method=tf.image.ResizeMethod.NEAREST_NEIGHBOR)]), axis=0)
    ...
    output_all = tf.reduce_mean(input_tensor=tf.stack([head_output, tail_output_rev]), axis=0)
...
def save_lip_images(image_path, samples, out_dir):
    img_A = cv2.imread(image_path).astype(np.float)
    rows = img_A.shape[0]
    cols = img_A.shape[1]
    image = samples[0]
    ...
    with open('{}/{}.txt'.format(out_dir, img_id), 'w') as f:
        for p in range(image.shape[2]):
            ...
            if channel_.shape[0] != rows or channel_.shape[1] != cols:
                ...
                channel_ = cv2.resize(channel_, [rows, cols], interpolation='nearest')
