...
DATA_LIST_PATH = './uploads/list/val.txt'
NUM_STEPS = 1 # Number of images in the validation set.
RESTORE_FROM = './checkpoints/JPPNet-s2'
...
def parse():
    ...
    parsing_out1 = tf.reduce_mean(input_tensor=tf.stack([tf.image.resize(parsing_out1_100, tf.shape(input=image_batch_origin)[1:3,]),
                                           tf.image.resize(parsing_out1_075, tf.shape(input=image_batch_origin)[1:3,]),
                                           tf.image.resize(parsing_out1_125, tf.shape(input=image_batch_origin)[1:3,])]), axis=0)
    parsing_out2 = tf.reduce_mean(input_tensor=tf.stack([tf.image.resize(parsing_out2_100, tf.shape(input=image_batch_origin)[1:3,]),
                                           tf.image.resize(parsing_out2_075, tf.shape(input=image_batch_origin)[1:3,]),
                                           tf.image.resize(parsing_out2_125, tf.shape(input=image_batch_origin)[1:3,])]), axis=0)
    parsing_out3 = tf.reduce_mean(input_tensor=tf.stack([tf.image.resize(parsing_out3_100, tf.shape(input=image_batch_origin)[1:3,]),
                                           tf.image.resize(parsing_out3_075, tf.shape(input=image_batch_origin)[1:3,]),
                                           tf.image.resize(parsing_out3_125, tf.shape(input=image_batch_origin)[1:3,])]), axis=0)

    raw_output = tf.reduce_mean(input_tensor=tf.stack([parsing_out1, parsing_out2, parsing_out3]), axis=0)
    ...
    raw_output_all = tf.reduce_mean(input_tensor=tf.stack([head_output, tail_output_rev]), axis=0)
